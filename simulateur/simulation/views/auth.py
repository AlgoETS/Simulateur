...
class JoinTeamView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if join_link.is_expired():
            return redirect('join_team', message='Link has expired')

        user = request.user
        user_profile = get_object_or_404(UserProfile, user=user)
        if user_profile.team:
            return redirect('team_dashboard', message='You are already part of a team')

        user_profile.team = team
        user_profile.save()
        team.members.add(user_profile)
        return redirect('team_dashboard', message='Successfully joined the team')